...
//TEST5
add('Fill in the blank. Do not use abbreviations. A network engineer is troubleshooting the configuration of new VLANs on a network. ​Which command is used to display the list of VLANs that exists on the switch?', 'show vlan');
...
add('Fill in the blank with an acronym. While configuring inter-VLAN routing on a multilayer switch, a/an','SVI');
add('A network administrator is verifying the configuration of inter-VLAN routing. Users complain that PC2 cannot communicate with PC1. Based on the output, what is the possible cause of the problem?','The encapsulation dot1Q 5 command contains the wrong VLAN.');
add('A network administrator is verifying the configuration of inter-VLAN routing. Users complain that PCs on different VLANs cannot communicate. Based on the output, what are two configuration errors on switch interface Gi1/1?','Gi1/1 is configured as trunk mode.', 'Gi1/1 is in the default VLAN.');
...
add('Open the PT Activity. Perform the tasks in the activity instructions and then answer the question. Fill in the blank. Do not use abbreviations. Which command is missing on the Layer 3 switch to restore the full connectivity between PC1 and the web server?','ip address 192.168.20.1 255.255.255.0');
...
add('Refer to the exhibit. Which command can the administrator issue to change the VLAN10 status to up?​','Switch1(config)# vlan 10');
...
add('Refer to the exhibit. In this network design, which connection or connections if any, add the VLAN ID number if host H1 sends information to host H2?','no link');
add('What is a characteristic of legacy inter-VLAN routing?','The router requires one Ethernet link for each VLAN.');
...
add('Match the inter-VLAN routing method to the corresponding characteristic the following order','router-on-a-stick -> creation of subinterfaces', 'Layer 3 with SVIs -> routing at wire speeds', '– not scored –', 'Layer 3 with routed ports -> need to issue the no switchport command');
