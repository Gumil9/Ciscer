...
add('Which three statements accurately describe VLAN types? (Choose three).',
	...
	'A management VLAN is any VLAN that is configured to access management features of the switch.','After the initial boot of an unconfigured switch, all ports are members of the default VLAN.');
...
add('What is the default DTP mode on Cisco 2960 and 3560 switches?',
	'dynamic auto');
add('What can be determined from the output that is shown?',
...
add('Consider the following routing table entry for R1: D 10.1.1.0/24 [90/2170112] via 209.165.200.226, 00:00:05, Serial0/0/0 What is the significance of the Serial0/0/0?',
	'It is the interface on R1 used to send data that is destined for 10.1.1.0/24.');
add('PC A sends a request to Server B. What IPv4 address is used in the destination field in the packet as the packet leaves PC A?',
	'192.168.12.16');
...
add('What does R1 use as the MAC address of the destination when constructing the frame that will go from R1 to Server B?',
	'If the destination MAC address that corresponds to the IPv4 address is not in the ARP cache, R1 sends an ARP request.');
add('If PC1 is sending a packet to PC2 and routing has been configured between the two routers, what will R1 do with the Ethernet frame header attached by PC1?',
...
add('dynamic auto');
add('nonegotiate');
add('dynamic desirable');
add('trunk');

add('User Priority – value that supports level or service implementation');
add('Type – value for the tag protocol ID value');
add('Canonical Format Identifier – identifier that enables Token Ring frames to be carried across Ethernet Links');
add('– not scored – -value for the application protocol of the user data in a frame');
